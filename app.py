...
# Callback to immediately update session state on table edit
def update_table_data():
    editor_key = f"editor_{selected_view}"
    if editor_key in st.session_state:
        changes = st.session_state[editor_key]
        
        # Determine active view filter
        if selected_view == "All Shafts Combined":
            current_df = st.session_state.df_master
        else:
            current_df = st.session_state.df_master[st.session_state.df_master["Sheet"] == selected_view]
            
        target_indices = current_df.index
        
        # Apply edits directly to master dataframe in session state
        for row_idx, col_changes in changes.get("edited_rows", {}).items():
            real_index = target_indices[row_idx]
            for col_name, val in col_changes.items():
                st.session_state.df_master.at[real_index, col_name] = val

# Recalculate dynamic 'Left' columns prior to rendering
...
# Interactive Data Editor with on_change Callback
st.data_editor(
    display_df,
    column_config={
